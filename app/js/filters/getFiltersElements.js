...
const getFiltersElements = () => {
    ...
    appliancesInFilter = [];
    utensilsInFilter = [];
    let ingredientsInFilterDOM = Array.from(document.querySelectorAll(".searchbox #ingredients-filter .filter-items .filter-item"));
    ingredientsInFilter = ingredientsInFilterDOM.map((ingredientInFilter) => {
        return ingredientInFilter.innerText;
    });
    let appliancesInFilterDOM = Array.from(document.querySelectorAll(".searchbox #appliances-filter .filter-items .filter-item"));
    appliancesInFilter = appliancesInFilterDOM.map((applianceInFilter) => {
        return applianceInFilter.innerText;
    });
    let utensilsInFilterDOM = Array.from(document.querySelectorAll(".searchbox #utensils-filter .filter-items .filter-item"));
